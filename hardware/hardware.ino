...
void checkPasscode(void){
    ...
    if(WiFi.status()== WL_CONNECTED){ 
      ...
      http.begin(client, "http://192.168.0.21:8080/api/check/combination"); // Your Domain name with URL path or IP address with path mona: 172.16.193.152
...
// Adafruit_ILI9341 tft = Adafruit_ILI9341(TFT_MOSI, TFT_CLK, TFT_DC, TFT_RST, TFT_CS);
...
//     // Map the potentiometer value to the range of 0-9 for digit display
//   int mapVal = map(potVal, 0, 4095, 0, 9);
...
//       // 1. REPLACE LOCALHOST IN THE STRING BELOW WITH THE IP ADDRESS OF THE COMPUTER THAT YOUR BACKEND IS aRUNNING ON
//       http.begin(client, "http://172.16.192.252:8080/api/check/combination"); // Your Domain name with URL path or IP address with path 
